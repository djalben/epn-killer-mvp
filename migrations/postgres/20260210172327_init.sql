...
CREATE TABLE cards (
    ...
    -- Баланс карты (для автопополнения)
...
CREATE TABLE transactions (
    ...
    transaction_type VARCHAR(50) NOT NULL, -- 'FUND', 'AUTH', 'CAPTURE', 'DECLINE'
...
-- Для частых поисков по user_id + статусу (например, "найти все активные карты пользователя")
CREATE INDEX idx_cards_user_status ON cards(user_id, card_status);
-- Для поиска транзакций по периоду (отчёты, фрод-мониторинг)
