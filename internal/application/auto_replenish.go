...
func processCardReplenishment(card models.Card) error {
	...
	if user.Balance.LessThan(card.AutoReplenishAmount) {
		log.Printf("[AUTO-REPLENISH] Insufficient user balance for card %d. User balance: %s, Required: %s",
			card.ID, user.Balance.String(), card.AutoReplenishAmount.String())
		...
		if user.TelegramChatID.Valid {
			notification.SendTelegramMessage(user.TelegramChatID.Int64,
				fmt.Sprintf("⚠️ *Auto-replenishment failed:* Недостаточно средств для пополнения карты `...%s`. Требуется: `%s`, доступно: `%s`",
					card.Last4Digits, card.AutoReplenishAmount.String(), user.Balance.String()))
		}
	...
	// 4. Списать с баланса пользователя
	_, err = tx.Exec(
